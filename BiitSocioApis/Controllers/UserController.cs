...
namespace BiitSocioApis.Controllers
{
    public class UserController : ApiController
    {
        ...
        public HttpResponseMessage LoginUser(User user)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var vs = db.students.Where(s => s.sonOf == user.CNIC).Select(s => new {std=s,us=db.Users.Where(m=>m.CNIC==s.cnic).SingleOrDefault() }).ToList();
                        if (vs.Count != 0)
                        {
                            user1 = new User();
                            user1.userType = "4";
                            ...
                            user1.password = user.password;
                            user1.name = user.name==null?"Parent":user.name;
                            user1.classAllowed = vs.Any(s => s.us.classAllowed == true);
                            user1.dateSheetAllowed = vs.Any(s => s.us.dateSheetAllowed==true);
                            user1.timeTableAllowed = vs.Any(s => s.us.timeTableAllowed == true);
                            user1.personalAllowed = vs.Any(s => s.us.personalAllowed == true);
                            user1.parentAllowedByTeacher = vs.Any(s => s.us.parentAllowedByTeacher == true);
                            user1.email = user.email == null ? "Email" : user.email;
                            user1.phone = user.phone == null ? "Phone" : user.phone;
                            if (!vs.Any(s => s.std.aridNo.ToLower().EndsWith(user.password.ToLower())))
                            {
                                var r = new
                                {
                                    statusCode = 300,
                ...
                if (user1 != null)
                {
                    ...
                    if (user1.userType == "4")
                    {
                        var vs = db.students.AsEnumerable().Where(s => s.sonOf == user.CNIC).Select(s => new { std = s, us = db.Users.AsEnumerable().Where(m => m.CNIC == s.cnic).SingleOrDefault() }).ToList();
                        user1.classAllowed = vs.Any(s => s.us.classAllowed == true);
                        user1.dateSheetAllowed = vs.Any(s => s.us.dateSheetAllowed == true);
                        user1.timeTableAllowed = vs.Any(s => s.us.timeTableAllowed == true);
                        user1.personalAllowed = vs.Any(s => s.us.personalAllowed == true);
      ...
      private bool  checkParentIfAdded(User user) {
            ...
            try
            {
                ...
                else if (user.userType != "1" && user.userType != "4")
                {
                    ...
                    if (fromWall == "5")
                    {
                        ...
                        var sections =user.userType=="2"? db.Allocations.Where(s => s.emp_no == cnic).Select(s => s.section.Trim()).Distinct().ToList():db.Allocations.Select(s=>s.section).Distinct().ToList();
                        foreach (var item in sections)
                        {
                            dd = new DropDownClass();
                            dd.category = item;
                            dd.users = db.Users.Join(db.students.Where(s => s.section == item), us => us.CNIC, std => std.cnic, (us, std) => us).ToList();
                            dd.data = new List<string>();
                            dd.isString=false;
                            data1.Add(dd);
                        }
        ...
        [HttpPost]
        public HttpResponseMessage setUserSettings(User u) {
            try
            {
                BIITSOCIOEntities db = new BIITSOCIOEntities();
                var user=db.Users.Where(s => s.CNIC == u.CNIC).SingleOrDefault();
                user.classAllowed = u.classAllowed;
                user.dateSheetAllowed = u.dateSheetAllowed;
                user.timeTableAllowed = u.timeTableAllowed;
                user.parentAllowedByTeacher = u.parentAllowedByTeacher;
                user.personalAllowed = u.personalAllowed;
                db.SaveChanges();
                return Request.CreateResponse(HttpStatusCode.OK,"Saved");
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK,"Something gone Wrong!");
            }
        }
