    ...
    public class PostController : ApiController
    {
        ...
        public HttpResponseMessage addPost()
        {
            ...
            try
            {
                ...
                var v = bool.Parse(request["isParentAllowed"]);
                post.isParentAllowed = bool.Parse(v.ToString());
        ...
        public HttpResponseMessage getPosts(string cnic,int pageNumber,string fromWall)
        {
            ...
            try
            {
                ...
                if (u.us.userType == "4")
                {
                    if (fromWall == "3")
                    {
                        List<String> childrensCnics = db.students.Where(s => s.sonOf.Trim() == cnic).Select(s => s.cnic).ToList();
                        foreach (var item in childrensCnics)
                        {
                            posts.AddRange(db.Posts.Where(p => p.status != "deleted" &&(p.isParentAllowed==null||p.isParentAllowed==true)&& p.fromWall == fromWall && (/*p.postFor.ToLower().Contains(item.ToLower())||*/ p.postFor.ToLower().Contains(cnic.ToLower()))).ToList());
                        }
                        posts = posts.Distinct().ToList();
                    ...
                    else if (fromWall == "2")
                    {
                            ...
                            posts.AddRange(db.Posts.Where(p => p.status != "deleted" && (p.isParentAllowed == null || p.isParentAllowed == true) && p.fromWall == fromWall && (p.postFor.ToLower().Contains(cnic.ToLower()) )).ToList());
                        
                        posts = posts.Distinct().ToList();
                    }
                    else if (fromWall == "6") {
                        var societies =  db.Societies.ToList();
                        posts = db.Posts.AsEnumerable().Where(p => p.fromWall == "6" && (p.isParentAllowed == null || p.isParentAllowed == true) && p.postFor.Split(',').Intersect(societies.Select(s => s.id.ToString())).Any()).ToList();
                    }
                    else if(fromWall=="1"){
                        List<String> childrensCnics = db.students.Where(s => s.sonOf.Trim() == cnic).Select(s => s.cnic).ToList();
                        foreach (var item in childrensCnics)
